###############################################################################################
####   Prepare Long Data Excel file for import to R and VolWQdb                            ####
####      Long data is data with duplicates or other QC in its own row on the spreadsheet  ####
###############################################################################################
...
# 3.  Sample metadata must include the fields below and their names should match exactly (see 5 for conditional metadata fields):
       ...
       # 'QCType' is DEQ nomenclature for S, FS, FP for the sample, required if duplicates don't have their own column.
...
# 4.  Each result field is designated by the prefix of the characteristic id (ca) and one or more suffixes.
      ...
      # Valid Values Charactersitic Abbreviations
       load("//deqlab1/wqm/Volunteer Monitoring/datamanagement/R/GrabWQ_DataMngmt/CharNames.RData") # Hope to not need path like //deqlab1/WQM/Volunteer Monitoring/datamanagement/R/VolGrabWQ/
       View(CharNames)  
       # The QC criteria for each characteristic also needs to be included in the QCcrit.csv file.
       QCcrit <- read.csv("//deqlab1/wqm/Volunteer Monitoring/datamanagement/R/GrabWQ_DataMngmt/QCcrit.csv",header=TRUE)  # criteria for grading
...
subid <- "0024" # NEEDS to be hand enetered here or added to the Excel file...shouldn't need to be a field in excel file uploaded to R
actorg <- "TWC" # Sampling orgnanization abreviation from volunteer database organization table
...
dir <- "//deqlead02/Vol_Data/WetlandConserv"  #INPUT the directory you want to retrieve and write files to, change the text in the quotes, leave out final '/'
file <- "TWC2015_4R.xlsx" # INPUT within the quotes the complete path of file
...
nr1 <- 115  # INPUT the number of rows in your csv file
...
# Reload these files in case they were not done during Excel file review
load("//deqlab1/wqm/Volunteer Monitoring/datamanagement/R/GrabWQ_DataMngmt/CharNames.RData")
QCcrit <- read.csv("//deqlab1/wqm/Volunteer Monitoring/datamanagement/R/GrabWQ_DataMngmt/QCcrit.csv",header=TRUE)
...
sufx <- c("_r", "_qual", "_PREC", "_ACC", "_DQL", "_m", "_cmnt") # suffixes to add to each Character ID
...
gdmeta <- c("LASAR", "SiteIDcontext","DateTime","DupBatchKey", "QCType", "item", "actroot")
...
gd<-gd[!duplicated(gd[ ,c(names(gd[1:(length(names(gd))-2)])) ]),] # clean duplicated rows from dataset
...
names(gdl)<- c(gdmeta,"r","rqual","prec", "acc","dql","mthd","cmnt","charid") # rename columns to generic names for all parameters
str(gdl)
...
for (x in 1:length(charsall)) {
  ...
  tmp <- gd[,charsall[[x]]]
  names(tmp) <- c(gdmeta,"r","rqual","prec", "acc","dql","mthd","cmnt")
...
# Create QC calc type for all field primary records
for (i in seq_along(gdl$r)) {
  gdl$QCcalc[i] <- if (gdl$QCType[i] %in% "S"){# if it is not a duplicate put "NA" in QCcalc
    NA
    ...
    "AbsDiff"
  } else "RPD"  #low level RPD's fixed later.
} 
...
#  Insert duplicate result and duplicat result qualifier column
gdl$dr <- as.numeric(NA)#length = length(gdl$LASAR))
gdl$dr_qual <- as.character(NA)
gdl$row <- seq_along(gdl$item) #Index numbers for reference 
gdl$fdrow <- as.numeric(NA) #place for index number to connect FP and FD

#  Assign duplicate result and result qualifiers to the duplicate columns for calculations in the FD rows
for (i in 1:length(gdl$item)){
  if (gdl$QCType[i] == 'FP') {
    dups<- gdl[Reduce(intersect, list(which(gdl$QCType == 'FD'),
                                      which(gdl$charid == gdl$charid[i]),
                                      which(gdl$LASAR == gdl$LASAR[i]))),]
    if (length(dups$LASAR) < 1) { # when no duplicates are found
      gdl$QCType[i] <- 'S'
      gdl$cmnt[i] <- ifelse(is.na(gdl$cmnt[i]), 'No duplicate reported for primary', paste0(gdl$cmnt[i],'; No duplicate reported for primary'))
      next
    }
    dups$td <- dups$DateTime - gdl$DateTime[i]
    #print(i)
    if (min(abs(dups$td))>30) {
      gdl$QCType[i] <- 'S'
      gdl$cmnt[i] <- ifelse(is.na(gdl$cmnt[i]), 
                         'No duplicate reported for primary within 30 min', 
                         paste0(gdl$cmnt[i],'; No duplicate reported for primary within 30 min'))
      next
    }
    gdl$dr[i] <- dups$r[which.min(abs(dups$td))]#print(dups$r[which.min(abs(dups$td))])#
    gdl$dr_qual[i] <- dups$rqual[which.min(abs(dups$td))]#print(dups$rqual[which.min(abs(dups$td))])
    gdl$fdrow[i] <- dups$row[which.min(abs(dups$td))]#
  }
}

# Correct QCcalc for RPD's with low level values to AbsDiff
for (i in 1:length(gdl$LASAR)) {
  if (gdl$QCcalc[i] == 'RPD' && !is.na(gdl$dr[i])) {
    if (mean(c(gdl$r[i],gdl$dr[i])) < (prj$LowLevQClimit[which(gdl$charid[i]==prj$CharID)])) gdl$QCcalc[i] <-'AbsDiff'  
  }
}
...
for (i in seq_along(gdl$r)) {
  gdl$prec_val[i] <- if (is.na(gdl$QCcalc[i])){
    NA
...
for (i in seq_along(gdl$r)) {
  gdl$DEQ_prec[i] <- if(is.na(gdl$QCcalc[i])){
    NA
...
# Assign prec_val and DEQ_prec to the FD data rows.
#  if there is a value in dr, then 
for (i in seq_along(gdl$r)) 
  if (!is.na(gdl$prec_val[i])){
    gdl$prec_val[gdl$fdrow[i]]<- gdl$prec_val[i]
    gdl$DEQ_prec[gdl$fdrow[i]]<- gdl$DEQ_prec[i]
  }



############   Tidy the dataset to keep only columns needed remove the duplicate data columns.

names(gdl)[which(names(gdl) %in% 'QCType')] <- 'ActType'

gdtidy <- gdl[,c("actroot","LASAR","SiteIDcontext","DateTime","DupBatchKey","item", "r","rqual", "prec","acc","dql","mthd","cmnt","charid","ActType","QCcalc","prec_val","DEQ_prec")]



# Save files for future use by subsequent data files 
